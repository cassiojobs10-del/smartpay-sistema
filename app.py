...
def gerir_colaborador(id_colab):
    ...
    if request.method == 'PUT':
        ...
        try:
            ...
            conexao.commit(); conexao.close(); return jsonify({"status": "sucesso"}), 200
        except sqlite3.IntegrityError:
            conexao.close(); return jsonify({"status": "erro", "mensagem": "Matrícula em uso."}), 400
    elif request.method == 'DELETE':
        cursor.execute('DELETE FROM colaboradores WHERE id = ?', (id_colab,)); conexao.commit(); conexao.close()
...
def metricas_dashboard():
    ...
    matricula = session['usuario']; data_inicio = request.args.get('inicio'); data_fim = request.args.get('fim')
    try:
        conexao = sqlite3.connect(NOME_BANCO); cursor = conexao.cursor()
...
def listar_remessas():
    if 'usuario' not in session: return jsonify({"status": "erro"}), 401
    conexao = sqlite3.connect(NOME_BANCO); conexao.row_factory = sqlite3.Row; cursor = conexao.cursor()
    cursor.execute('SELECT * FROM lancamentos_remessas WHERE matricula_usuario = ? ORDER BY id DESC', (session['usuario'],))
...
def salvar_ocorrencia():
    if 'usuario' not in session: return jsonify({"status": "erro"}), 401
    dados = request.json; conexao = sqlite3.connect(NOME_BANCO); cursor = conexao.cursor()
